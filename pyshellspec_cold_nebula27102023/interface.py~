...
class Interface(object):
    ...
    def compute_chi2(self, pars=[], fitparams=None, verbose=False):
        ...
        self.limcofcp= limcof.Limcof(Teff=self.__model['central_object']['tstar'].to('Kelvin').value, R=self.__model['central_object']['rstar'].to('solRad').value, M=self.__model['central_object']['emstar'].to('solMass').value)
...
#                    print("compdict = ", compdict[key])  # dbg
    ...
    def __run_shellspec(self):
        ...
        # added dust opacities
        self.__symlink('../dust_opac', 'dust_opac')
        
        
        # run shellspec
        cmd = [os.path.join(cwd, 'shellspec')]
        #print(subprocess.check_output("ls -l; pwd", shell=True).decode())
