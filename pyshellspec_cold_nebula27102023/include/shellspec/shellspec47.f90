...
!        
	...
	real(i8):: xunt1,yunt1,xunt2,yunt2,xunt3,yunt3,rmdfx4
        real(i8):: rmdfx1,rmdfx2,rmdfy1,rmdfy2,rmdfz1,rmdfz2,rmdfx3
        real(i8):: stepf,stepfz,gainfx,gainfy,gainfz
        real(i8):: rmdx1,rmdx2,rmdy1,rmdy2,rmdz1,rmdz2,rmdz3,rmdz4
        real(i8):: steps,stepsz,gainx,gainy,gainz
	...
	real(i8):: tempjt,densjt,anejt,vtrbjt,dstdjt,dsttjt
	...
	integer:: nfreq,nbodfa,nbodfb,nbodf1,nbodf2,nbodf3
	integer:: nbod1,nbod2,nbod3,nboda,nbodb
...
!for Slavek	
!	open(18,file='sh.txt',status='unknown')
!	open(19,file='shx.txt',status='unknown')
...
!         the box of the body frozen frame (if imodel=1) [R_sol]
!       rmdfx3<rmdfx4 -define an additional box on the x-coordinate 
!         It will have the same (rmdfy1,rmdfy2,rmdfz1,rmdfz2) dimension.
!         rmdfx1<rmdfx2<rmdfx3<rmdfx4 all in R_sol
!       stepf -is a mean distance between the x,y grid points [R_sol]
!       stepfz -is a mean distance between the z grid points [R_sol]
!         They determine the number of grid points:
!       nbodf1, nbodf2, nbodf3 -number of grid points in x, y, z (>=1)
!         direction in body frozen coordinates of the model.
!         (Points are overridden 
!         by the values from `shellspec.mod' if imodel=2)
!       gainfx, gainfy, gainfz -grid step multiplication factors
!         of the body frozen grid to allow for logarithmic grid 
...
!       rmdx1<rmdx2, rmdy1<rmdy2, rmdz1<rmdz2 - define the box
!         of the observer's line of sight frame. 
!       rmdz3<rmdz4 -define an additional box on the z-coordinate.
!         rmdz1<rmdz2<rmdz3<rmdz4 all in R_sol
!         Observer looks along the opposite z-direction.
!       steps -is a mean distance between the x,y grid points [R_sol]
!       stepsz -is a mean distance between the z grid points [R_sol]
!         They determine the number of grid points:
!       nbod1, nbod2, nbod3 -number of grid points in x, y, z (>=1)  
!         in the line of sight observer's frame.
!       gainx, gainy, gainz -grid step multiplication factors
!         (common ratio of the geometric sequence) of the line of sight
!         grid    [gainx=(x_{i+1}-x_{i})/(x_{i}-x_{i-1})]
...
!       itdc=1  disc temperature is constant (=tempdc)
!       itdc=2  disc temperature is a function of r (accretion discs)
!               T(r)=tempdc*(rdc/r)**0.75*(1-(rdc/r)**0.5)**0.25       
!       itdc=3  disc temperature as a power law (e.g. protopl. discs)
!               T(r)=tempdc*(r/rindc)**etmpdc
...
!               dust temperatures behave like gas temperatures for
!               different itdc but with dsttdc instead of tempdc 
!               it must be higher than the condensation temperature
!               of some species (see dust_opac) to have an effect
...
!       tempjt -temperature [K], constant in the jet
...
!       dsttjt -dust temperature [K], constant in the jet
!               it must be higher than the condensation temperature
!               of some species (see dust_opac) to have an effect
...
!       ituf=3  disc temperature as a power law (e.g. protopl. discs)
!               T(r)=tempdc*(r/rindc)**etmpuf
...
!	only ipart=1 is suppoted at the moment
	ipart=1
...
!-----------------------------------------------------------------------
!	The shell (input) is defined in its body frozen cartesian 
...
!	Shell is first inclined around 'ar' by dinc degrees and 
...
!	be cautious imodel=2 does not support the gap in the coordinates
!       and use only nbodfb=0 before this option	
...
!		untransparent object before do 190 cycle??
...
570	continue
!	for Slavek fluxn was defined a field
!	do nu=1,nfreq
!	  write(18,520)(idint(fluxn(nu,iang)*1.d4),iang=1,nphase)
!	  write(19,'(f12.4)')alam(nu)/1.d1
!520       format(300i6)	  
!	enddo
...
! Beware: split2 does NOT work for all steps!!!
!          call split2(opdept(id1),opdept(id1+1),opslim,sf2(id1),       &
!     &      sf2(id1+1),cf12)

! Beware: this criterion does NOT work for a spot embedded in the disk
! because even with previous depth being >10 (opdlim), the contrib. f
! is be HUGE, because one multiplies by huge opdepd; takes 2.1 sec
!         if(opdept(id1+1).lt.opdlim.and.opdepd.gt.opslim)then

! This seems to work (all large tau are split); takes 2.7 sec
          if(opdepd.gt.opslim)then
            call split2(opdept(id1),opdept(id1+1),opslim,sf2(id1),     &
     &      sf2(id1+1),cf12)
!            write(*,*) 'split2!'  ! dbg
          else
...
!            write(*,*) 'opdepd = ', opdepd
!            write(*,*) ' > opslim = ', opslim
!            write(*,*) 'opdept(id1+1) = ', opdept(id1+1)
!            write(*,*) ' < opdlim = ', opdlim
!            stop
!          endif
          aint=cf12+aint
...
!-----------------------------------------------------------------------
	...
	implicit none
        integer, parameter:: i1=1,i4=4,i8=8
        include 'param.inc'
	integer:: nbodf1,nbodf2,nbodf3,nbodfa,nbodfb
	...
	real(i8):: rosue(ndimf1,ndimf2),roteme(ndimf1,ndimf2)
        ...
        real(i8):: denxnb(mstarx),denznb(mstarx)
	real(i8):: rmdfx1,rmdfx2,rmdfy1,rmdfy2
        real(i8):: rmdfz1,rmdfz2,rmdfx3,rmdfx4
        real(i8):: gainfx,gainfy,gainfz
...
!		definition of the model body frozen grid
...
!-----------------------------------------------------------------------
     ...
     &  ,gainx,gainy,gainz,ar,at,az)
!       definition of the line of sight grid
!	it allows to define/merge two grids along the z axis
...
!          of the interval to the left and to the right
	implicit none
        integer, parameter:: i4=4,i8=8
        real(i8):: ar(ndim1),at(ndim2),az(ndim3)
	integer:: ndim1,ndim2,ndim3,nbod1,nbod2,nboda,nbodb
	real(i8):: rmdx1,rmdx2,rmdy1,rmdy2,rmdz1,rmdz2,rmdz3,rmdz4
	real(i8):: gainx,gainy,gainz
	real(i8):: r1,rn,rstep,st1,t1,tn,tstep,z1,zn,zstep
	integer:: n2,i,n2b
!	geom. sequence assumes that nbod1,nbod2,nboda,nbodb are odd        
!		x  
...
!	it allows to define/merge two grids along the x axis
!	gain?=1 grid is equidistant
!       gain?>1. step increases geometrically from the center 
!          of the interval to the left and to the right
	implicit none
        integer, parameter:: i4=4,i8=8
        real(i8):: ar(ndim1),at(ndim2),az(ndim3)
	integer:: ndim1,ndim2,ndim3,nboda,nbod2,nbod3,nbodb
	real(i8):: rmdx1,rmdx2,rmdy1,rmdy2,rmdz1,rmdz2,rmdx3,rmdx4
	real(i8):: gainx,gainy,gainz
	real(i8):: r1,rn,rstep,st1,t1,tn,tstep,z1,zn,zstep
	integer:: i,n2,n2b
!	geom. sequence assumes that nbod2,nbod3,nboda,nbodb are odd        
!		x
   	if(gainx.eq.1.d0)then
          r1=rmdx1
 	  rn=rmdx2 	
