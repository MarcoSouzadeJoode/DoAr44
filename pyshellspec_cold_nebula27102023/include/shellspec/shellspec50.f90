...
!		behind the last untransparent object along the ray
     ...
     &        ,lunt3,nstar3,xstar3,star3,aint0,aintb)
              ...
              if (ii.eq.(nbod1/2+1).and.jj.eq.(nbod2/2+1)) then
              !print*, '--- LOGS FROM EDIT ON 25 Oct 2023 ---'
              !print*,'aintb = ', aintb
              !print*,'ii = ', ii
              !  print*,'jj = ', jj
              !  print*, 'rstar = ',rstar/rsol
              !  print*, 'size of pixelx = ', stepx
              !  print*,'area of pixel = ', stepx*stepy
              !  print*,'area of star = ',  pi * (rstar/rsol)**2 / (stepx*stepy)
              !  print*,'ratio of areas = correction = ', (pi * (rstar/rsol)**2) / (stepx*stepy)

                if ((rstar/rsol).le.stepx) then
                    aintb = aintb * pi * (rstar/rsol)**2 / (stepx*stepy)
              !      print*,'aintb = ', aintb  
                endif
              !  print*, ' --- END OF LOGS FROM EDIT ON 25 Oct 2023 ---'
                
              endif
             
         
              
! MARCO : VRÁCENO aintb - což je ta intenzita
! ii == N/2
! jj == N/2
! ozkoušet střední pixel
! nbod1
! nbod2
...
!	  	heart of iteration
	    ...
	    !print*,' ==== ekonc, endpf = ', ekonc, ' ', endfp
	    if(dabs(dx/ekonc).lt.xacc)goto 45
	    if(i.eq.niter)then
	      !print*, ' ==== LOGS FROM 27 OCT 2023 ==== '
	      !print*, 'MARCO : ekonc : ', ekonc
	      !print*, 'MARCO :  xacc : ', xacc
	      !print*, 'MARCO :  dx   : ', dx
	      !print*, 'MARCO :  dx/ekonc : ', dx/ekonc
	      !write(*,*)' error, slow/no convergence in sub.: elnd'
	      !write(3,*)' 1 error, slow/no convergence in sub.: elnd'
	      !print*, ' ==== END LOGS FROM 27 OCT 2023 ==== '
	      ! call exit(1)
	      
	      !print*, 'WARNING : non standard behaviour. setting ekonc = 0 manually '
	      ekonc = 1.d-50
	      goto 45
	    endif
          enddo
